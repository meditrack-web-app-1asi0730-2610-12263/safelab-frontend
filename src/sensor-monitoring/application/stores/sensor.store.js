  ...
  getters: {
    filteredSensors(state) {
      ...
      const byQuery = normalizedQuery
        ? state.sensors.filter(sensor => sensor.name.toLowerCase().includes(normalizedQuery))
        : state.sensors
      ...
      return [...byType].sort((left, right) => {
        const leftTime = left.installedAt ? left.installedAt.getTime() : 0
        const rightTime = right.installedAt ? right.installedAt.getTime() : 0
